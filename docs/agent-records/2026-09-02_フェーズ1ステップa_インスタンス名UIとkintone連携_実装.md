...
- ~~明細テーブルの「変更後商品単位」（`fidChangedProductUnit`）に対応するkintoneフィールドが見つからない。現状は空欄のまま。ユーザーに別途確認が必要（固定文言でよいか、他アプリに情報があるか等）~~ → **解消（2026-09-02）**：実機確認TC-26でユーザーが空欄表示のまま運用上問題ないことを確認。対応不要と判断。**さらに2026-09-11、業務支援チームとの調整により`fidChangedProductUnit`・`fidAddedProductUnit`パーツ自体がフォームから削除され、根本的に解消（[実施記録](2026-09-11_単位パーツ削除の反映.md)参照）**
