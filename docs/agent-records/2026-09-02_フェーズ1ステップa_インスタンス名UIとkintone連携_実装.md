...
type: task-record
date: 2026-09-02
status: open   # 実機確認・未解決課題が残るためopen
...
- `kintone-contract-db`エンドポイントのquery固定値（テスト用顧客番号）が未設定のため、検証環境での実機動作確認が未実施。設定完了後にユーザーが確認する
- フェイルセーフ（DOM構造不一致時の申請ブロック）自体の動作確認（意図的にID/クラス名を壊すテスト）も、上記の実機確認と合わせて検証環境で実施予定
