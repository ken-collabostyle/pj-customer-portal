...
**2026-09-02追記**：クラウド契約管理DBと商品マスタは直接連携しておらず、間に「ライセンス種類マスタ」というアプリを挟んでいることが判明。このアプリはまだフィールド定義（`getFormFields`生レスポンス）を取得・整理できておらず、本フォルダに対応表は未作成。業務支援チームとkintone側のアプリ構造自体の見直しを検討中のため、商品マスタへの新規アクセスを伴う実装は保留中（詳細は[全体計画](../plans/2026-08-24_顧客ポータルカスタマイズ全体計画.md)「商品マスタ関連の保留事項」参照）。構造が確定した段階で、必要であれば本フォルダにライセンス種類マスタの対応表を追加する。
